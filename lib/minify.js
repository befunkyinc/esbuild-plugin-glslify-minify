...
class GlslMinify {
  ...
  executeFile(input) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      // Open issue: https://github.com/leosingleton/webpack-glsl-minify/issues/19
      if (this.options.preserveUniforms) {
        for (const uniformName in uniforms) {
          if (uniformName !== uniforms[uniformName].variableName) {
            throw new Error(`Uniform name collision: "${uniformName}" -> "${uniforms[uniformName].variableName}"`);
          }
        }
      }
  ...
  minifier(content) {
    ...
    // Remove "#define GLSLIFY 1"
    if (this.options.removeGlslifyDefinition) {
      const firstLine = '#define GLSLIFY 1\n';
      if (output.startsWith(firstLine)) {
        output = output.slice(firstLine.length);
      }
    }
